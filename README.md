...
Generated by `node scripts/make-tray-icons.mjs`. The icons are build output, not
hand-drawn assets.

### 🔴 The source is a tile inside a tile

`icon.png` is a **512px cream square containing a 320px black rounded tile**,
inset 96px on every side. Scaled straight to 16px the tray showed a cream frame
around the real icon — and wasted a third of the pixels at the size where pixels
are scarcest. The app icon fed to the installer and the Start Menu had exactly
the same defect, since it was a straight copy of that file.

⚠️ **The cream cannot be removed by colour: the B inside the tile is also cream.**
Knocking out every cream pixel erases the letter and leaves a black lozenge. The
cream to remove is the cream *outside* the tile, and "outside" is topological,
not chromatic — so it is found by flood-filling inward from the four corners.
Cream enclosed by black is never reached. The tile's own corner radius survives
untouched, because it is preserved rather than re-cut.

Both the tray icons and `build/icon.png` now come from that cropped tile (320px,
above electron-builder's 256 minimum, real pixels rather than an upscale).

### ⚠️ Measured contrast, and the trade it exposes

| icon | vs light taskbar | vs dark taskbar |
|---|---|---|
| white silhouette (first attempt) | **0%** | high |
| cream-framed app icon | 42% | 67% |
| tight black tile (now) | **95%** | **13%** |

On a light taskbar the black tile reads. On a dark taskbar the tile blends and
**the cream B inside it is what you see** — 13% of pixels, roughly thirty at
16px, which is a legible glyph rather than nothing. Both themes show something;
neither shows everything.

That is not a fixable number, it is the shape of the problem: no single opaque
image is high-contrast against both a near-white and a near-black background.
The previous cream-framed version scored more evenly (42/67) precisely because it
had both a light field and a dark tile — and it looked wrong. If the dark-taskbar
result is not good enough in practice, the fix is a per-theme icon pair, not a
different crop.

### macOS

Unchanged: the monochrome silhouette as a **template image**, black on
transparent, inverted by the OS. Only idle is a template — a template is
rendered as a mask, so a coloured state dot inside one would come out an
indistinguishable blob.

### State dot

Green ringing (the answer colour), blue on-a-call, bottom-right, separated from
the mark by a transparent ring. Both stay clear of the presence vocabulary's red
(deliberately off) and amber (broken): these are activity states and must not
borrow a meaning that already belongs to something else.
